...
import pandas as pd
from find_units import search_line,find_units_from_line


DB_loc = r'XL_DVH'
res_file = 'ETC_lists.xlsx'
...
with open('all_lists.txt') as f:
    for text_line in f:
        rowN_in_file +=1       
        rowN_in_res +=1
        
        if 'COUNTRY_TAG' in text_line:
            country_tag=text_line.split()[1]
        if 'ROOSTER_START_FACTION' in text_line:
            faction_tag=text_line.split()[1]                        
            faction_name = find_match(df_DIM_FACTION,'FA_KEY_P','faction_name',faction_tag)            
            player_id +=1
